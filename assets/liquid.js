...
class LiquidEther {
  ...
  init() {
    ...
    class WebGLManager {
      constructor(props) {
        ...
        this._onVisibility = () => {
          ...
          if (hidden) {
            this.pause();
          } else if (this.props.refs.isVisibleRef) {
            this.start();
          }
        };
      ...
      loop() {
        ...
        this.render();
        this.props.refs.rafRef = requestAnimationFrame(this._loop);
      }
      start() {
        if (this.running) return;
        this.running = true;
      ...
      pause() {
        this.running = false;
        if (this.props.refs.rafRef) {
          cancelAnimationFrame(this.props.refs.rafRef);
          this.props.refs.rafRef = null;
        }
      }
      dispose() {
        try {
    ...
    const io = new IntersectionObserver(
      entries => {
        ...
        this.refs.isVisibleRef = isVisible;
        if (!this.webglRef) return;
  ...
  dispose() {
    if (this.refs.rafRef) cancelAnimationFrame(this.refs.rafRef);
